...
Vector2f pca(std::vector< ParametrizedLine<float,2> >& bin) {
  ...
  float cvar = 0.0;
  for(auto &b : bin) {
    cvar += (b.pointAt(0)(0) - avg(0)) * (b.pointAt(0)(1) - avg(1));
  }
  cvar /= bin.size();
  MatrixXf cvarmat;
  cvarmat.resize(2,2);
...
bool approx_same(ParametrizedLine<float,2>& a, ParametrizedLine<float,2>& b, float smax_angle, float cmax_angle, float max_dist) {
  return (std::abs(a.distance(b.pointAt(0))) / (a.pointAt(0) - b.pointAt(0)).norm() < 0.5 * smax_angle and
          std::abs((a.pointAt(1) - a.pointAt(0)).dot((b.pointAt(1) - b.pointAt(0)))) > cmax_angle and
          (a.pointAt(0) - b.pointAt(0)).norm() < max_dist);
}
...
void raw_to_wall(std::vector<float>& vertices, std::vector<float>& normals, std::vector<float>& crvts, std::vector< ParametrizedLine<float,2> >& walls, float crvt_max, float nz_max, float min_dist_from_ceil_and_floor) {
  ...
  for (int i = 1; i < vertices.size()/3; ++i) { // find height of ceiling and floor
    ceilheight = vertices[3*i + 2] > ceilheight ? vertices[3*i + 2] : ceilheight;
    floorheight = vertices[3*i + 2] < floorheight ? vertices[3*i + 2] : floorheight;
  }
  for (int i = 0; i < vertices.size()/3; ++i) { // do filtering
    if (std::abs(crvts[i]) < crvt_max and
        std::abs(normals[3*i + 2]) < nz_max and
        vertices[3*i + 2] < ceilheight - min_dist_from_ceil_and_floor and
        vertices[3*i + 2] > floorheight + min_dist_from_ceil_and_floor) {
